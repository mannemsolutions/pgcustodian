...
func TestNewClientFromEnv(t *testing.T) {
	...
	assert.Equal(t, secretIdFile, client.SecretIDFile, fmt.Sprintf("client.SecretIDFile should default to %s", secretIdFile))
	...
	assert.Equal(t, defaultStore, client.StorePath, fmt.Sprintf("client.StorePath should default to %s", defaultStore))
...
func TestConnectRoleID(t *testing.T) {
	...
	t.Logf("SecretIDFile: %s", client.SecretIDFile)
	err := client.Connect()
	assert.NoError(t, err, "login to vault should succeed")
	assert.NotEmpty(t, client.token, "After connect with vault, there should be a token")
	assert.NotNil(t, client.client, "After connect with vault, there should be vault client registered")
